...
public class RunTestsMojoTest extends MojoTest {
	...
	*/
	...
	public void testRunTestsMojoBasic() throws Exception {
		...
		this.expectedFailsafeSummaryXml =
				this.expectedFailsafeSummaryXml.replace(">3<", ">1<").replace(">6<", ">2<").replace("\r\n", "\n");
	...
	public void testRunTestsMojoCreateSymLink() throws Exception {
		...
		verify(mojo.fitNesseHelper, times(1)).launchFitNesseServer(WikiMojoTest.PORT_STRING, mojo.workingDir, mojo.root, mojo.logDir,
				mojo.authentication);
		verify(mojo.fitNesseHelper, times(1)).createSymLink(mojo.project.getBasedir(), mojo.testResourceDirectory, WikiMojoTest.PORT,
				launch);
	...
	public void testCreateSymLinkException() throws Exception {
		doThrow(new IOException("TEST")).when(mojo.fitNesseHelper).launchFitNesseServer(anyString(), anyString(), anyString(),
				anyString(), anyString());
