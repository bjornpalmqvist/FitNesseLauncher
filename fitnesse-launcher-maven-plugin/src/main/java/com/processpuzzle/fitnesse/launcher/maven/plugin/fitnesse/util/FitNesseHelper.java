...
public class FitNesseHelper {
	...
	*/
	public void createSymLink(final File basedir, final String testResourceDirectory, final int port, final Launch... launches)
		throws IOException {
	...
	public Process forkFitNesseServer(final String port, final String workingDir, final String root, final String logDir,
	...
	public void launchFitNesseServer(final String port, final String workingDir, final String root, final String logDir,
	...
	private ArrayList<String> buildCommandLineArgumentsAsArray(final String port, final String workingDir, final String root) {
		ArrayList<String> commandLineArguments =
				Lists.newArrayList("-e", "0", "-o", "-p", String.valueOf(port), "-d", workingDir, "-r", root);
		return commandLineArguments;
	}

	private int createSymLink(final File basedir, final String testResourceDirectory, final int port, final String linkName)
		throws IOException {
		final String linkPath = calcLinkPath(linkName, basedir, testResourceDirectory);

		HttpURLConnection connection = null;
		try {
			final String urlPath = String.format("/root?responder=symlink&linkName=%s&linkPath=%s&submit=%s",
	...
	*/
	...
	private String calcLinkPath(final String linkName, final File basedir, final String testResourceDirectory)
		...
		final StringBuilder linkPath =
				new StringBuilder(basedir.toURL().toString().replaceFirst("/[A-Z]:", "").replaceFirst(":", "://"));
